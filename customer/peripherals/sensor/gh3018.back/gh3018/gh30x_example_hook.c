...
void Gh30xGetRawdataHookFunc(GU32 *read_buffer_ptr, GU16 length)
{
    ...
    uint32_t buffer2[sample_num];
    for (uint8_t i = 0; i < length; i++)
    {
        index = i / 2;
        if (i % 2 == 0)
        {
            uint32_t ppg = (read_buffer_ptr[i] >> 7) & 0x1FFFF; // Bit[23:7]: Rawdata
            buffer[index] = ppg;
            EXAMPLE_DEBUG_LOG_L1_HOOK("[%s]buf[%d]=%d\n\r", __FUNCTION__, index, buffer[index]);
        }
        else
        {
            uint32_t ppg = (read_buffer_ptr[i] >> 7) & 0x1FFFF; // Bit[23:7]: Rawdata
            buffer2[index] = ppg;
            EXAMPLE_DEBUG_LOG_L1_HOOK("[%s]buf2[%d]=%d\n\r", __FUNCTION__, index, buffer2[index]);
        }
    }
