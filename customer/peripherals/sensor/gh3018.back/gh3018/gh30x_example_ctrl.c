...
* Input:  None
* Return: Sample rate(Hz)  正常配置采样率应该大�?25Hz，且�?25的整数�?
******************************************************************/
static GU16 HBD_AutoLedCalcuSampleRate()
{
    ...
    if (0 == usRegVal)
    {
        usSampleVal = HB_SAMPLING_RATE_SETTING; //默�?��?
    }
    else
    {
...
//        g_unIdealTargetTrigArr[uchIndex] = g_unIdealTargetTrigArr[uchIndex];
    ...
    //正常配置采样率应该大�?25Hz，且�?25的整数�?(当通信错�??采样率算错再�?3�?)
    if (!usSampleVal) //如果采样率为0
    {
        ...
        if (!usSampleVal) //3次重新�?�算之后采样率了依然为零 上报错�??
        {
            return HBD_RET_COMM_ERROR;
        }
    }
    ...
    g_stSampleInfo.unSampleFifoDuration = 0; //初�?�化一�?fifo�?�?对应的时�?
    g_stSampleInfo.unAutoLedCalcLen = usSampleVal / 5;  //计算触发调光的长�?
    ...
    g_uchAutoLedLastFlag = 0; //调光标�??

    /*单帧模式 */

    g_uchSoftLEDNewdataModeCNT = 0;//计数�?
    ...
    GU8 uchCurrentArr[2], GU8 uchCurrentShift[2], GU32 unLastCurrentMsk)
{
    ...
    HBD_I2cWriteRawdataReg(HBD_LED12_REG_ADDR, unLastCurrentMsk | (GU32)(uchCurrentArr[0] << uchCurrentShift[0]) | (GU32)(uchCurrentArr[1] << uchCurrentShift[1]));//写电�?
...
//            *fstd = *fstd + (GF32)(uNum - 1) / uNum * (luinput - *favg) * (luinput - *favg);//递归计算标准�?
//            *favg = *favg + (GF32)(luinput - *favg) / uNum;//递归计算均�?
//        }
//        *ludiff = *lumax - *lumin;
//    }
//}
...
//    GS16 sACCY = 0;
//    GS16 sACCZ = 0;
//    if (HBD_PTR_NULL == stGsAxisValue)//空指�? 返回0
//    {
//        pstaccstd->fAvg = 0;
//        pstaccstd->uMax = 0;
//        pstaccstd->uMin = 0;
...
//    {
//        sACCX = stGsAxisValue[i].sXAxisVal / sGsensorRightShift;//X轴归一�?
//        sACCY = stGsAxisValue[i].sYAxisVal / sGsensorRightShift;//Y轴归一�?
//        sACCZ = stGsAxisValue[i].sZAxisVal / sGsensorRightShift;//Z轴归一�?
//        unACC = (GU32)sqrt(sACCX * sACCX + sACCY * sACCY + sACCZ * sACCZ);//矢量�?
//        HBD_AutoLedRecursionCalc(unACC, &pstaccstd->fStd, &pstaccstd->fAvg, &pstaccstd->uMin, &pstaccstd->uMax, &pstaccstd->uDif, i + 1);//计算标准�?
//    }

//    if (1 == lLength || 0 == lLength){
//        pstaccstd->fStd = 0;//长度小于1 标准�?�?0
//    }
//    else{
//        pstaccstd->fStd = sqrtf(pstaccstd->fStd / (lLength - 1));//递归标准�?�?�? STD = SQRT(STD(t-1)/(NUM-1))
//    }
    //LOGY("");
...
{
    for (GU8 i = 0; i<HBD_AUTO_LED_MAX_CHANNEL_NUM; i++)//对每�?通道单独调用接口计算均值最大值最小�?
    {
        //用于计算rawdata的均�?
...
static void HBD_AutoLedPpgSatJudge(GU32 uRawdata[2])
{
    ...
    GU32 Upper = 0;
    for (GU8 i = 0; i<HBD_AUTO_LED_MAX_CHANNEL_NUM; i++)//对每�?通道单独调用接口计算饱和数据的个�?
    {
        Upper = g_stAutoLedConfig.stAutoLedChannel[i].unTrigerUpperThr;//外部配置触发上限

        switch (g_stBGAdj.stBGAdjCh[i].uchUpperlevel)//根据背景光切换触发阈�?
        {
            case 0:
            break;
            case 1://对应3-4之间的背�?�?
        ...
        g_PpgSatThr[i]= Upper;  ////�?改饱和阈值为触发上限
...
//    gs_flag2 = g_stPpgSatInfo.Sat_cnt[1];
    ...
    GU32 SAT_CNT_THRESHOLD = SAT_CHECK_NUM_MAX/16;    //  饱和判断阈值：1/16的数�?

    if (g_stPpgSatInfo.Sat_CheckCnt >= SAT_CHECK_NUM_MAX)
    {
        g_stPpgSatInfo.Sat_flag = 0 ; //一次统计（5秒）之后初�?�化饱和标志 ，饱和标致置位的话会持续5�?
        for (GU8 j = 0; j<HBD_AUTO_LED_MAX_CHANNEL_NUM; j++)//对每�?通道单独调用接口计算饱和数据的个�?
        {
    ...
    GU8 *puchCurrentArr, GU8 *puchCurrentShift, GU32 *punLastCurrentMsk)
{
    ...
    *punLastCurrentMsk = HBD_I2cReadRawdataReg(HBD_LED17_REG_ADDR);//读取电流寄存�?
    *pusLastGainMsk = HBD_I2cReadReg(HBD_LED27_REG_ADDR);//读取Gain寄存�?

    for (uchIndex = 0; uchIndex < g_uchNeedReadRawdataCount; uchIndex++)
    {
        puchCurrentShift[uchIndex] = (GU8)((g_usReadRawdataRegList[uchIndex] - HBD_LED0_DATA_L_REG_ADDR) * 2);//计算通道偏移�?
        puchCurrentArr[uchIndex] = (*punLastCurrentMsk >> puchCurrentShift[uchIndex]) & 0xFF;//通道电流�?
        g_uchAutoLedCurrentArr[uchIndex] = puchCurrentArr[uchIndex];  //给NADT提供的电流�?
        *punLastCurrentMsk &= ~(0x000000FF << puchCurrentShift[uchIndex]);//通道Mask

        puchGainShiftArr[uchIndex] = (GU8)((((g_usReadRawdataRegList[uchIndex] - HBD_LED0_DATA_L_REG_ADDR) / 4) * 3) + 4);//计算通道偏移�?
    ...
    else if (1 == g_uchNeedReadRawdataCount)//电流�?0 返回异常
    {
...
static GU8 HBD_AutoLEDBGProcess(GU8 uchGainArr[2], GU32 unRawdataAvgArr[2])
{
    ...
    for (GU8 uchChannelIndex = 0; uchChannelIndex < g_uchNeedReadRawdataCount; uchChannelIndex++)
    {
        ...
        {//�?在通道开�?调光时进�?
            ...
            BGAvg = g_stBufferBGInfo[uchChannelIndex].fAvg;//均�?
            BGMin = (GU8)g_stBufferBGInfo[uchChannelIndex].uMin;//最小�?

            if (BGAvg >= g_stAutoLedConfig.uchBgStrongLightThr)//强光(BG5 最大范�?:32768-49152)
            {
            ...
            else if (BGAvg > g_stAutoLedConfig.uchBgWeakStrongLightThr && BGAvg < g_stAutoLedConfig.uchBgStrongLightThr && BGMin >= g_stAutoLedConfig.uchBgWeakStrongLightThr)//弱强�?
            {//BG4-5     Max:49151
                ...
                if ( (GU32)g_stTrigCalcInfo[uchChannelIndex].fAvg > 46000) //BG 4-5,Max:49151 90% -->46000
                {
                    if (uchGainArr[uchChannelIndex] >= 1)
                    {
                        ...
                        g_stAutoLedConfig.stAutoLedChannel[uchChannelIndex].uchGainLimitThr = uchGainArr[uchChannelIndex]; //限制Gain
                        g_uchAutoLedLastFlag |= 1 << uchChannelIndex; //调光标�??
                        uchRes = 1;
                    }
                    else{
...
//        uchMotionLevel = HBD_AUTOLED_MOTION_LEVEL_2;//步�??
//    }
//    else{
...
static GU8 HBD_AutoLedRawdataAvgDeal(GU32 uRawdataAvg[2])
{
    ...
    for (GU8 i = 0; i<g_uchNeedReadRawdataCount; i++)
    {
      if (g_stAutoLedConfig.stAutoLedChannel[i].emAutoledEnable == HBD_FUNCTIONAL_STATE_ENABLE)
      {
        ...
        /* Average*/
        //数据饱和�?(饱和标志一旦置�?5秒切�?一次状�?)，考虑大值�?�结果的影响  调光均值�?�算偏大  一旦数�?饱和就生�?(优先限制数据饱和)
...
static GU8 HBD_AutoLedWriteRegConfirm(GU8 uchCalcGain, GU8 uchCalCurrent, GU8 uchGainArr[2], GU8 uchCurrentArr[2], GU8 uchchannel)
{
    ...
    {//Gain*电流 判断向上调节还是向下调节 之前一段时间有触发上限 不能�?
    ...
    return (GU8)(g_stAutoledWrtRegCheck.unWrtRegNormDuration[uchchannel] / g_stAutoledWrtRegCheck.unWrtRegThres);//返回标�??
}
    ...
    GU8 uchCurrentArr[2], GU8 uchCurrentShift[2], GU32 unLastCurrentMsk, GU32 unRawdataAvgArr[2])
{
    ...
    GU8 uchCurrentLimit = 0;//功耗限�?
    GU8 uchGainLimit = 0;//gain限制(因为背景�?
    GU8 uchAdjFlag = 0;//调节标�??
    GU8 uchtrigger = 0;//触发理想�?标值调节标�?
    ...
    for (uchChannelIndex = 0; uchChannelIndex < g_uchNeedReadRawdataCount; uchChannelIndex++)
    {
        ...
        if (!(uchFlag >> uchChannelIndex & 0x1))
        {
            if (g_stAutoLedConfig.stAutoLedChannel[uchChannelIndex].emAutoledEnable == HBD_FUNCTIONAL_STATE_ENABLE)
            {
                ...
                //if (fRatio > gfMotionTrigRatio[uchChannelIndex]) /* 大于理想�?标值调节比�?(此�?�叠加了运动的影�?,所以为Motion) */
                if (uchTrigFlg)
                {
                    /* 计算�?否进入调�? */
                    if (!g_stIdealAdjust.stIdealAdjustCh[uchChannelIndex].uchFirstTrigger) /* �?否�??一次触�?*/
                    {
                        uchtrigger = 1;
                    ...
                    {
                        if (g_stIdealAdjust.stIdealAdjustCh[uchChannelIndex].uchLastIdealTrigger) /* �?加到周期�?*/
                        {
                            g_stIdealAdjust.stIdealAdjustCh[uchChannelIndex].uchTriggerCnt += g_stSampleInfo.unSampleFifoDuration;
                            uchtrigger = (GU8)(g_stIdealAdjust.stIdealAdjustCh[uchChannelIndex].uchTriggerCnt / g_stIdealAdjust.uchTriggerDuration / 1000);//�?否触发标�?
                        }
                        else{
                            uchtrigger = 0;
                        }
                    ...
                    if (uchtrigger)
                    {
                        ...
                        if (uchGainArr[uchChannelIndex] >= uchGainLimit || uchCurrentArr[uchChannelIndex] >= uchCurrentLimit)
                        {
                            ...
                            {//大于功耗限制，增大Gain，重新�?�算电流
                                ...
                                {
                                    uchCalcCurrent = uchCurrentLimit; //电流最大为功耗限制�?
                                }
                            }
                ...
                else
                {
                    ...
                    //复位后�??一次没触发理想值调节则不再进�?��??一次触发判�?，避免初期�?�繁调光影响出值时�?
...
static float HBD_AutoLedfIpdSaturation(GU8 uchGain, GU8 uchCurrent, GU32 unRawdataAvgArr, GU8 uchAmp)
{
    ...
    float fIpd = 0.0f;
    fSatRatio = ((float)unRawdataAvgArr - 60000.0f) / 60000.0f;//�?考虑过饱和的情况
    fSatRatio = fSatRatio > 0.0f ? 1 + uchAmp * fSatRatio : 1.0f; // �?针�?�过饱和，其它情况不考虑
...
static GU8 HBD_AutoLedDirectCalculate(GU8 uchchindex, GF32 fIpd, GU8 uchGain, GU8 uchcurrent, GU16 *usPrefCurrent, GU8 uchlimitcur,GU32 unidealtarget, GU32 unrawedataavg)
{
    ...
    if (fIpd != 0)
    {
        usCalcCur = (GU16)(unidealtarget / fIpd / g_usGainArrEnum[uchGain]);//计算电流，注意�?��?�为U16
    }
    else
    {
...
{
    GU8 uchCalcFlag = 0;
    GU8 uchCalcGain = *uchGain;//另�?�算Gain初值为当前寄存器的�?
    ...
    while (uchCalcFlag)//当结果不�?0 根据结果选择Gain++ 或者Gain-- 后重新�?�算
    {
    ...
    GU8 uchLimitGain, GU32 uIdealRawdata, GU32 uRawdataAvg, GU32 lnTakeoff, GU8 uchGain, GU8 uchCur)
{
    ...
    switch (uchcmpflag)
    {
        ...
        case HBD_AUTO_LED_COMPATIBLE_CASE2: //电流达到限制值，Gain等于限制�?
            if (uchCur == 0xFF && uchGain == uchLimitGain)//最大电�? 最大限制Gain
            {
                if (uRawdataAvg < lnTakeoff)
                {
                    uchRes = HBD_AUTO_LED_COMPATIBLE_CASE2;//最大电流最大限制Gain,小于脱落阈�? 失败
                }
                ...
                uCalcRawdata = (GU32)(fIpd * uchLimitCurrent * g_usGainArrEnum[uchLimitGain]);//用限制值�?�算rawdata
    ...
    GU8 uchCurrentArr[2], GU8 uchCurrentShift[2], GU32 unLastCurrentMsk, GU32 unRawdataAvgArr[2])
{
    ...
    g_uchAutoLedErrorFlag = 0;
     ...
     /*�?一步理想目标值触发判�? 当有通道非rawdata触发上下限时进入 */
    if (0x3 != uchFlag)
    {
        ...
        for (uchChannelIndex = 0; uchChannelIndex < g_uchNeedReadRawdataCount; uchChannelIndex++)
        {
            ...
            if ((uchFlag & (1 << uchChannelIndex)) != 0) // ch1~ch2
            {
                ...
                {//进入兼�?�模式�?�理
                    ...
                    {
                        if (g_stBGAdj.uchTurnOnNotReturnFail)//不返回失败开�?
                        {
...
GU8 HBD_AutoLedbyNewDataInt(GU32 unRawdata[10])
{
    ...
    /* PPG Sat judge   饱和判断需要根�?背景光等级改变饱和阈值的*/
    ...
    if (g_uchSoftLEDNewdataModeCNT == (g_stSampleInfo.unSampleFre - 1))
    {
        ...
        if (HBD_AutoLEDBGProcess(uchAutoLedGainArr, unAutoLedRawdataAvgArr))
        {
          ...
          /* Saturation Process   数据饱和�?改理想目标值触发阈值修�?*/
        ...
        HBD_AutoLedPpgSatTrigerFlag(&uchAutoLedFlag, unAutoLedRawdataAvgArr);    //判断�?否饱和触发调�?
...
GU8 HBD_AutoLedbyFifoInt(GU32 *punDataBuffer, GU16 usRawdatabufferCnt)
{
    ...
    for (GU16 cnt = 0; cnt < usRawdatabufferCnt; cnt++)
    {
        ...
        /* AutoLED Flag */
        if (cnt >= usCalcStartIndex)   //�?取中�?最近的5帧数�?处理
        {
    ...
    { //// 没有寄存器的电流值为0，�?�算CTR过大，则判断指夹�?的调光失�?
        ...
        for (i = 0; i < g_uchNeedReadRawdataCount; i++)
        {
            GU32 nCuurent = uchAutoLedCurrentArr[i] * usMaxDriveCur; // nCuurent/255为电�?(单位: uA);nCuurent/255=0(整形相除),计算CTR时把除以255改为乘以255
    ...
    /* Rawdata Trigger 调光触发上阈值会根据背景光等级修�? */
    uchAutoLedFlag = HBD_AutoLedCalcTrigerFlag();
                                           ...
                                           GS32 nRawDataArrForAlgo[10], GS32 nRawDataArrForDbg[][6], GU16 *pusRawdataOutLenForDbg, GU16 usDbgMaxLen)
{
    ...
    if (stGsensorDataArr != HBD_PTR_NULL)
    {
        ...
        {
            //Gsensor数据比PPG数据少的话就进�?�差值和重采样�?�理补齐
...
* Input:nSpO2Arr[0]：�?�氧值，返回�?70-100�?
        nSpO2Arr[1]：�?�氧置信等级；
        nSpO2Arr[2]：�?�氧实时置信度�?
        nSpO2Arr[3]：�?�氧平均R�?*10000�?
        nSpO2Arr[4]：红外心率值；
        nSpO2Arr[5]：红外心率置信度�?
        nSpO2Arr[6]：数�?无效标�?�，无效（调光或运动）时�?1
        nSpO2Arr[7]：红外PI�?*100000
        nSpO2Arr[8]：红光PI�?*100000
        nSpO2Arr[9]：红外SNR�?*10
        nSpO2Arr[10]：红光SNR�?*10
        nSpO2Arr[11]：�?�氧实时R�?*10000
******************************************************************/
