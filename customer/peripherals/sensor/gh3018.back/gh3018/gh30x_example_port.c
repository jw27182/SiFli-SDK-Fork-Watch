                            ...
                            uint16_t length)
{
    ...
    if (gh3018_i2cbus)
    {
        msgs[0].addr = gh3018_get_dev_addr();         /* Slave address */
                           ...
                           uint16_t read_length)
{
    ...
    if (gh3018_i2cbus)
    {
        ...
        msgs[1].addr = gh3018_get_dev_addr(); /* Slave address */
...
static void gh30x_sensor_task(void *params)
{
    ...
    #endif
#endif
        LOG_D("gh30x_sensor_task: IRQ re-enabled. Waiting for next interrupt...");
    }
}
...
void *hal_gh30x_memory_malloc(GU32 size)
{
    ...
    return malloc(size);
#else
    void *pmem = rt_malloc(size);
    if (NULL == pmem)
    {
        EXAMPLE_DEBUG_LOG_L1("malloc %d Btyes failed\n", size);
    }else {
        EXAMPLE_DEBUG_LOG_L1("malloc %d Btyes success\n", size);
    }
    return pmem;
    #endif
}
...
{
// check register first, register 1 should init to 0x17
