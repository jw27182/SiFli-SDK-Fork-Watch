...
void Gh30xSampleResetHandle(stSampleResetCfg *pstSampleResetCfg)
{
    ...
    {
        Gh30xSetDefaultDrvCurrent(0,
                                  pstSampleResetCfg->uchResetDrvCurrentChnl0);
    }
    if (pstSampleResetCfg->usResetEn & RESET_CFG_CHNL1_DEFAULT_CUREET_EN)
    {
        Gh30xSetDefaultDrvCurrent(1,
                                  pstSampleResetCfg->uchResetDrvCurrentChnl1);
    }
    ...
    for (GU8 uchFunCnt = 0; uchFunCnt < GH30X_FUNC_OFFSET_MAX; uchFunCnt++)
    {
        if (g_pstGh30xFrameInfo[uchFunCnt])
        {
            if (unFunc & (((GU32)1) << uchFunCnt))
            {
                ...
                g_pstGh30xFrameInfo[uchFunCnt]->pstFunctionInfo->uchDivderCnt =
                    0;
            }
        }
    }
...
void Gh30xDemoFuncEnable(GU32 unFuncMode)
{
    ...
    EXAMPLE_DEBUG_LOG_L1("[%s] %x, %x\r\n", __FUNCTION__, g_unGh30xDemoFuncMode,
                         unRealFuncModeNeedOpen);
    ...
    {
        EXAMPLE_DEBUG_LOG_L1("[%s] from NULL/ADT mode to Main func mode, we "
                             "need diable ADT function\r\n",
                             __FUNCTION__);
...
void Gh30xDemoFuncDisable(GU32 unFuncMode)
{
    if (g_unGh30xDemoFuncMode &
        (~GH30X_FUNCTION_ADT)) // from Main func mode to ADT mode, we need reset
                               // ADT function
    {
        g_uchGh30xIsAdtMode = 1;
        GH30X_AlgoDeinit(GH30X_FUNCTION_ADT);
...
void gh30x_module_start(GU32 unFunctionMode)
{
    ...
    if (GH30X_FUNCTION_HRV &
        unFunctionMode) // if HRV is include, sample rate is 100 Hz
    {
        stSampleResetCfgTemp.usResetEn =
            RESET_CFG_SAMPLE_RATE_EN | RESET_CFG_FIFO_WATER_LINE_EN;
        stSampleResetCfgTemp.usResetSampleRate = 100;
        // stSampleResetCfgTemp.uchResetFifoWaterLine = 50; // 25 *
        // g_uchNeedReadRawdataCount, default 50
        stSampleResetCfgTemp.uchResetFifoWaterLine =
            4; // 2 * g_uchNeedReadRawdataCount = 4
    }
    else if (GH30X_FUNCTION_ADT == unFunctionMode)
    {
        ...
        stSampleResetCfgTemp.uchResetFifoWaterLine =
            25; // 25 * g_uchNeedReadRawdataCount, default 25
    }
    else
    {
        stSampleResetCfgTemp.usResetEn =
    ...
    if (GH30X_FUNCTION_SPO2 & unFunctionMode) // unFunctionMode has sop2, select
                                              // spo2 cfg array priority
    {
        Gh30xDemoFuncEnable(unFunctionMode);
        Gh30xDemoSampleStart(
            spo2_reg_config_array, CONFIT_SIZE(spo2_reg_config_array),
            g_stGh30xSpo2ExtralCfg, CONFIT_SIZE(g_stGh30xSpo2ExtralCfg),
            &stSampleResetCfgTemp);
    }
    else if ((GH30X_FUNCTION_HR | GH30X_FUNCTION_HRV |
              GH30X_FUNCTION_SOFT_ADT) &
             unFunctionMode) // unFunctionMode has
    {
        Gh30xDemoFuncEnable(unFunctionMode);
        Gh30xDemoSampleStart(
            hb_reg_config_array, CONFIT_SIZE(hb_reg_config_array),
    ...
    else if (GH30X_FUNCTION_ADT == unFunctionMode)
    {
        Gh30xDemoFuncEnable(unFunctionMode);
        Gh30xDemoSampleStart(
            hb_adt_confirm_reg_config, CONFIT_SIZE(hb_adt_confirm_reg_config),
            g_stGh30xAdtExtralCfg, CONFIT_SIZE(g_stGh30xAdtExtralCfg),
            &stSampleResetCfgTemp);
    }
    else
    {
        EXAMPLE_DEBUG_LOG_L1("[%s] unFunctionMode is invalid! Please cheak. "
                             "unFunctionMode = 0x%X.\r\n",
...
void Gh30xFunctionFrameInfoUpdate(GU32 unFunctionMode,
                                  stSampleResetCfg *stSampleCfg)
{
    ...
    {
        if (((0x1 << i) & unFunctionMode) &&
            g_pstGh30xFrameInfo[i]->pstFunctionInfo->usOutputDataRate >
                stSampleCfg->usResetSampleRate)
        {
...
void Gh30xFunctionFrameInfoBaseInit(void)
{
    ...
    g_pstGh30xFrameInfo[GH30X_FUNC_OFFSET_ADT]
        ->pstFunctionInfo->usOutputDataRate = 25;
    g_pstGh30xFrameInfo[GH30X_FUNC_OFFSET_ADT]->puchChnlMap[0] =
    ...
    g_pstGh30xFrameInfo[GH30X_FUNC_OFFSET_HRV]
        ->pstFunctionInfo->usOutputDataRate = 100;
    g_pstGh30xFrameInfo[GH30X_FUNC_OFFSET_HRV]->puchChnlMap[0] =
        CHNL_MAP_TO_FIFO_PHASE0_DATA;
#endif

#if __FUNC_TYPE_SPO2_ENABLE__
    g_pstGh30xFrameInfo[GH30X_FUNC_OFFSET_SPO2]->pstFunctionInfo->uchChnlNum =
        2;
    g_pstGh30xFrameInfo[GH30X_FUNC_OFFSET_SPO2]
        ->pstFunctionInfo->usOutputDataRate = 100;
    g_pstGh30xFrameInfo[GH30X_FUNC_OFFSET_SPO2]->puchChnlMap[0] =
        CHNL_MAP_TO_FIFO_PHASE0_DATA;
    g_pstGh30xFrameInfo[GH30X_FUNC_OFFSET_SPO2]->puchChnlMap[1] =
        CHNL_MAP_TO_FIFO_PHASE1_DATA;
#endif

#if __FUNC_TYPE_SOFT_ADT_ENABLE__
    g_pstGh30xFrameInfo[GH30X_FUNC_OFFSET_SOFT_ADT]
        ->pstFunctionInfo->uchChnlNum = 2;
    g_pstGh30xFrameInfo[GH30X_FUNC_OFFSET_SOFT_ADT]
        ->pstFunctionInfo->usOutputDataRate = 25;
    g_pstGh30xFrameInfo[GH30X_FUNC_OFFSET_SOFT_ADT]->puchChnlMap[0] =
        CHNL_MAP_TO_FIFO_PHASE0_DATA;
    g_pstGh30xFrameInfo[GH30X_FUNC_OFFSET_SOFT_ADT]->puchChnlMap[1] =
        CHNL_MAP_TO_FIFO_PHASE1_DATA;
#endif
    ...
    g_pstGh30xFrameInfo[GH30X_FUNC_OFFSET_PROTOCOL]
        ->pstFunctionInfo->uchChnlNum = 2;
    g_pstGh30xFrameInfo[GH30X_FUNC_OFFSET_PROTOCOL]
        ->pstFunctionInfo->usOutputDataRate =
        200; // if usOutputDataRate is bigger than chip sample rate, uchDivder
             // is 0 or 1
    g_pstGh30xFrameInfo[GH30X_FUNC_OFFSET_PROTOCOL]->puchChnlMap[0] =
        CHNL_MAP_TO_FIFO_PHASE0_DATA;
    g_pstGh30xFrameInfo[GH30X_FUNC_OFFSET_PROTOCOL]->puchChnlMap[1] =
        CHNL_MAP_TO_FIFO_PHASE1_DATA;
#endif
}

void GH30X_RegisterHookFunc(
    void (*pInitHookFunc)(void),
    void (*pGetRawdataHookFunc)(GU32 *puchReadDataBuffer, GU16 usReadDataLen),
    void (*pGetIoStructInfoHookFunc)(
...
/// gsnesor fifo
// static ST_GS_DATA_TYPE
// gsensor_soft_fifo_buffer[GSENSOR_SOFT_FIFO_BUFFER_MAX_LEN];
...
int gh30x_module_init(void)
{
    ...
    EXAMPLE_DEBUG_LOG_L1("%s", hbd_ver);
    EXAMPLE_DEBUG_LOG_L1(GH30X_EXAMPLE_VER_STRING);
    EXAMPLE_DEBUG_LOG_L1("SDK code version: %s\r\n", HBD_GetHbdVersion());
    ...
    if (first_init_flag)
    {
        ...
        HBD_SetI2cRW(
            HBD_I2C_ID_SEL_1L0L, gh30x_i2c_write_exchange_to_spi,
            gh30x_i2c_read_exchange_to_spi); // register i2c exchange to spi api
#elif (__GH30X_COMMUNICATION_INTERFACE__ == GH30X_COMMUNICATION_INTERFACE_I2C)
        hal_gh30x_i2c_init(); // i2c init
        HBD_SetI2cRW(HBD_I2C_ID_SEL_1L0L, hal_gh30x_i2c_write,
                     hal_gh30x_i2c_read); // register i2c RW func api
#else
        EXAMPLE_DEBUG_LOG_L1(
            "[gh30x_module_init] communication interface undefine!");
#endif

#if (__PLATFORM_DELAY_US_CONFIG__)
        HBD_SetDelayUsCallback(hal_gh30x_delay_us);
#endif
        GH30X_RegisterHookFunc(Gh30xInitHookFunc, Gh30xGetRawdataHookFunc,
                               Gh30xFrameDataHookFunc);
    }
 ...
 *                             1, Less than level 0 memory required, HBwithHRV
 *function will unsupport. Return: size in bytes
 ******************************************************************/
...
void gh30x_reset_evt_handler(void)
{
    ...
    if ((reinit_ret == HBD_RET_OK) &&
        (gh30x_run_mode != RUN_MODE_INVALID)) // if reinit ok, restart last mode
    {
#if 0
    #if (__USE_GOODIX_APP__)
...
static void gh30x_fifo_evt_getrawdata_mode_only(GS32 *dbg_rawdata_ptr)
{
    ...
    GU8 nRes = 0;
    nRes = HBD_GetRawdataByFifoInt((GU8)__GET_RAWDATA_BUF_LEN__,
                                   (GS32(*)[2])rawdata, &dbg_rawdata_len);
    ...
    if (wearing_state == WEAR_STATUS_UNWEAR)
    {
        SEND_MCU_HB_MODE_RESULT(0, 0, WEAR_STATUS_UNWEAR, 0, 0, 0, 0,
                                currentarr,
                                (GS32(*)[DBG_MCU_MODE_PKG_LEN])dbg_rawdata_ptr,
                                dbg_rawdata_len, 0, 0, 0, 0);
        HBD_Stop();
        gh30x_handle_calc_unwear_status();
    }
    else
    {
        HAL_GH30X_FIFO_INT_TIMEOUT_TIMER_START();
        SEND_MCU_HB_MODE_RESULT(0, 0, WEAR_STATUS_WEAR, 0, 0, 0, 0, currentarr,
                                (GS32(*)[DBG_MCU_MODE_PKG_LEN])dbg_rawdata_ptr,
                                dbg_rawdata_len, 0, 0, 0, 0);
    }
...
static void gh30x_fifo_evt_factory_mode_calc(GS32 *dbg_rawdata_ptr)
{
    ...
    HBD_GetRawdataByFifoInt(GH30X_RAW_DATA_LEN, puchRawdataBuffer,
                            &usRawdataLen); // read FIFO
    ...
    if (usRawdataLen > 0)
    {
        if (gstFactoryModeCfg.now_state == FACTORY_GREEN_IR_STATE)
        {
            ...
            for (GU8 uchIndex = 0; uchIndex < usRawdataLen; uchIndex++)
            {
                g_UNGh30xRawdataBuf.stFactoryRawdata.gre1Buf[uchIndex] =
                    puchRawdataBuffer[uchIndex][0] & 0xFFFFFF; // green
                g_UNGh30xRawdataBuf.stFactoryRawdata.IrBuf[uchIndex] =
                    puchRawdataBuffer[uchIndex][1] & 0xFFFFFF; // ir
            }
        }
        else if (gstFactoryModeCfg.now_state == FACTORY_IR_RED_STATE)
        {
            ...
            {
                g_UNGh30xRawdataBuf.stFactoryRawdata.IrBuf[uchIndex] =
                    puchRawdataBuffer[uchIndex][0] & 0xFFFFFF; // ir
                g_UNGh30xRawdataBuf.stFactoryRawdata.redBuf[uchIndex] =
                    puchRawdataBuffer[uchIndex][1] & 0xFFFFFF; // red
            }
            handle_factory_mode_result(&g_UNGh30xRawdataBuf.stFactoryRawdata);
            memset(&g_UNGh30xRawdataBuf.stFactoryRawdata, 0,
                   sizeof(g_UNGh30xRawdataBuf.stFactoryRawdata));
        }
        else
        {
            return;
...
static void gh30x_fifo_evt_hr_spo2_mode_calc(GS32 *dbg_rawdata_ptr)
{
    ...
    HBD_HrSpo2CalculateByFifoIntEx(
        gsensor_soft_fifo_buffer, gsensor_soft_fifo_buffer_index,
        __GS_SENSITIVITY_CONFIG__,
        (GS32(*)[DBG_MCU_MODE_PKG_LEN])dbg_rawdata_ptr, &dbg_rawdata_len,
        &stSpo2Res);

    handle_spo2_mode_result(&stSpo2Res,
                            (GS32(*)[DBG_MCU_MODE_PKG_LEN])dbg_rawdata_ptr,
                            dbg_rawdata_len);
    EXAMPLE_DEBUG_LOG_L1(
        "spo2 calc, gs_len=%d, result=%d,%f,%d,%d,%d,%d,%d\r\n",
        gsensor_soft_fifo_buffer_index, stSpo2Res.uchSpo2, stSpo2Res.fSpo2,
        stSpo2Res.uchHbValue, stSpo2Res.uchHbConfidentLvl, stSpo2Res.usSpo2RVal,
        stSpo2Res.uliSpo2InvlaidFlag, stSpo2Res.uchWearingState);

    if (stSpo2Res.uchWearingState == WEAR_STATUS_UNWEAR)
    {
        SEND_MCU_SPO2_UNWEAR_EVT((GS32(*)[DBG_MCU_MODE_PKG_LEN])dbg_rawdata_ptr,
                                 dbg_rawdata_len);
        HBD_Stop();
        gh30x_handle_calc_unwear_status();
    }
    else
    {
        ...
        SEND_MCU_SPO2_MODE_RESULT(
            &stSpo2Res, abnormal_state, currentarr,
            (GS32(*)[DBG_MCU_MODE_PKG_LEN])dbg_rawdata_ptr, dbg_rawdata_len);
    }
...
static void gh30x_fifo_evt_spo2_mode_calc(GS32 *dbg_rawdata_ptr)
{
    ...
    HBD_Spo2CalculateByFifoIntDbgRawdataInnerUse(
        gsensor_soft_fifo_buffer, gsensor_soft_fifo_buffer_index,
        __GS_SENSITIVITY_CONFIG__, &spo2_value, &spo2_lvl, &hb_value, &hb_lvl,
        &hrv_val[0], &hrv_val[1], &hrv_val[2], &hrv_val[3], &hrv_lvl, &hrv_cnt,
        &spo2_r_value, &pliSpo2InvalidFlag, &wearing_state,
        (GS32(*)[DBG_MCU_MODE_PKG_LEN])dbg_rawdata_ptr, &dbg_rawdata_len,
        &valid_lvl);
    handle_spo2_mode_result(spo2_value, spo2_lvl, hb_value, hb_lvl, hrv_val,
                            hrv_lvl, hrv_cnt, spo2_r_value, wearing_state,
                            (GS32(*)[DBG_MCU_MODE_PKG_LEN])dbg_rawdata_ptr,
                            dbg_rawdata_len);
    // EXAMPLE_DEBUG_LOG_L1("fspo2 : %f\r\n", fSpo2Value);
    EXAMPLE_DEBUG_LOG_L1(
        "spo2 calc, gs_len=%d, result=%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\r\n",
        gsensor_soft_fifo_buffer_index, spo2_value, spo2_lvl, hb_value, hb_lvl,
        hrv_val[0], hrv_val[1], hrv_val[2], hrv_val[3], hrv_lvl, hrv_cnt,
    ...
    else
    {
        ...
        SEND_MCU_SPO2_MODE_RESULT(
            spo2_value, spo2_lvl, hb_value, hb_lvl, hrv_val, hrv_lvl, hrv_cnt,
            spo2_r_value, wearing_state, valid_lvl, abnormal_state, currentarr,
            (GS32(*)[DBG_MCU_MODE_PKG_LEN])dbg_rawdata_ptr, dbg_rawdata_len);
    }
    EXAMPLE_LOG_RAWDARA("spo2 calc:\r\n", dbg_rawdata_ptr, dbg_rawdata_len);
}
...
static void gh30x_fifo_evt_hrv_mode_calc(GS32 *dbg_rawdata_ptr)
{
    ...
    HBD_HbWithHrvCalculateByFifoIntDbgData(
        gsensor_soft_fifo_buffer, gsensor_soft_fifo_buffer_index,
        __GS_SENSITIVITY_CONFIG__, &hb_value, &hb_value_lvl, &wearing_state,
        &wearing_quality, &voice_broadcast, hrv_rr_value_array,
        &hrv_rr_value_fresh_cnt, &hrv_lvl,
        (GS32(*)[DBG_MCU_MODE_PKG_LEN])dbg_rawdata_ptr, &dbg_rawdata_len);
    if (wearing_state == WEAR_STATUS_UNWEAR)
    {
        ...
        SEND_MCU_HRV_MODE_RESULT(hrv_rr_value_array, 0, 0, 255, currentarr, 0,
                                 0);
        gh30x_unwear_evt_handler();
    }
    else
    {
        HAL_GH30X_FIFO_INT_TIMEOUT_TIMER_START();
        handle_hrv_mode_result(
            hrv_rr_value_array, hrv_rr_value_fresh_cnt, hrv_lvl,
            (GS32(*)[DBG_MCU_MODE_PKG_LEN])dbg_rawdata_ptr, dbg_rawdata_len);
        handle_hb_mode_result(hb_value, 0, WEAR_STATUS_WEAR, 0, 0, 0);
        SEND_MCU_HRV_MODE_RESULT(hrv_rr_value_array, hrv_rr_value_fresh_cnt,
                                 hrv_lvl, hb_value, currentarr,
                                 (GS32(*)[DBG_MCU_MODE_PKG_LEN])dbg_rawdata_ptr,
                                 dbg_rawdata_len);
        EXAMPLE_DEBUG_LOG_L1("hrv calc, result=%d,%d,%d,%d,%d\r\n",
                             hrv_rr_value_array[0], hrv_rr_value_array[1],
                             hrv_rr_value_array[2], hrv_rr_value_array[3],
...
void Gh30xNormalizeGsensorSensitivity(ST_GS_DATA_TYPE gsensor_buffer[],
                                      GU16 gsensor_point_num)
{
    GU8 uchRightShift;
...
void GH30xGetFrameDataAndProcess(GU8 *puchRawdataBuf,
                                 GU16 usRawdataSamplePointNum,
                                 ...
                                 float fGensorStep, GU16 usFrameNum)
{
    ...
    // set incomplete flag
    for (GU16 usSearchStartPosi = 0;
         usSearchStartPosi < usRawdataSamplePointNum; usSearchStartPosi++)
    {
        ...
        if (0xFF != uchChnlId)
        {
            ...
            // fill rawdata
            pstFrameInfo->punRawdata[uchChnlId] =
                ((GU32 *)puchRawdataBuf)[usSearchStartPosi];
        }

        if (uchCompeletMask ==
            (uchCompeletMask & uchTempIncompeletFlag)) // got one frame
        {
            if ((0 == pstFrameInfo->pstFunctionInfo->uchDivder) ||
                (0 == pstFrameInfo->pstFunctionInfo->uchDivderCnt))
            {
                ...
                for (GU8 uchChnlCnt = 0; uchChnlCnt < uchChnlNum; uchChnlCnt++)
                {
                    ...
                    pstFrameInfo->pstFunctionInfo->uchFrameFlag &=
                        (~(3 << uchleftMoveBitNum));
                    pstFrameInfo->pstFunctionInfo->uchFrameFlag |=
                        (g_pstGh30xChnlInfo[uchLogicChnlId].uchLedColor
                         << uchleftMoveBitNum);
                    // EXAMPLE_DEBUG_LOG_L1("color = %d\r\n", )
                    // fill drv current
                    pstFrameInfo->pchDrvCurrentCode[uchChnlCnt] =
                        g_pstGh30xChnlInfo[uchLogicChnlId].uchDrvCurrentCode;
                }
                // file gsenor data
                pstFrameInfo->pusGsensordata[0] =
                    pusGsValueArr[((GU16)fGsensorIndex) * 3];
                pstFrameInfo->pusGsensordata[1] =
                    pusGsValueArr[((GU16)fGsensorIndex) * 3 + 1];
                pstFrameInfo->pusGsensordata[2] =
                    pusGsValueArr[((GU16)fGsensorIndex) * 3 + 2];

                // get algo io data hook
                memset(pstFrameInfo->pstAlgoResult, 0,
                       sizeof(STGh30xAlgoResult));
            ...
            {
                if (pstFrameInfo->pstFunctionInfo->uchDivder <=
                    (pstFrameInfo->pstFunctionInfo->uchDivderCnt + 1))
                {
                          ...
                          const STGh30xFrameInfo *const pstFrameInfo)
{
    ...
    // EXAMPLE_DEBUG_LOG_L1("[GH30xFunctionProcess] %d++++++\r",
    // pstFrameInfo->pstFunctionInfo->uchDivder);
    ...
    // EXAMPLE_DEBUG_LOG_L1("[GetFrameNum]FunctionID: %d FrameNum: %d,
    // usGsDataNum: %d, GsensorStep: %.3f\r\n",(int)pstFrameInfo->unFunctionID,
    // usFrameNum, usGsDataNum, fGsensorStep);

    /***  get one frame data and process***/
    GH30xGetFrameDataAndProcess(puchRawdataBuf, usRawdataSamplePointNum,
                                pusGsValueArr, usGsDataNum, pstFrameInfo,
...
void Gh30xUpdateFunctionDivder(void)
{
    ...
    for (GU8 uchFunCnt = 0; uchFunCnt < GH30X_FUNC_OFFSET_MAX; uchFunCnt++)
    {
        ...
        {
            GU32 unTemp;
            GU16 usSampleRateAfterDiv = g_pstGh30xFrameInfo[uchFunCnt]
                                            ->pstFunctionInfo->usOutputDataRate;
            ...
            {
                unTemp = (GU32)32768 * 2 / usRegValue / usSampleRateAfterDiv +
                         1; // half adjust
                g_pstGh30xFrameInfo[uchFunCnt]->pstFunctionInfo->uchDivder =
                    ((GU8)(unTemp / 2));
                EXAMPLE_DEBUG_LOG_L1(
                    "[%s]FUN_ID = 0x%X, ChipSampleRate = %d, "
                    "usSampleRateAfterDiv = %d, uchDivder = %d.\r\n",
                    __FUNCTION__, g_pstGh30xFrameInfo[uchFunCnt]->unFunctionID,
            ...
            {
                EXAMPLE_DEBUG_LOG_L1(
                    "[%s]FUN_ID = 0x%X. ChipSampleRate = %d, usOutputDataRate "
                    "is zero. \r\n",
                    __FUNCTION__, g_pstGh30xFrameInfo[uchFunCnt]->unFunctionID,
                    ((((GU32)32768 * 2 / (GU32)usRegValue) + 1) / 2));
            }
...
void Gh30xDemoFunctionProcess(GU8 *puchReadFifoBuffer, GU16 usFifoBuffLen,
                              ST_GS_DATA_TYPE *pstGsAxisValueArr,
                              GU16 usGsDataNum)
{
    // copy raw data to send buffer
#if __USE_GOODIX_APP__
    GH30xFunctionProcess(puchReadFifoBuffer, usFifoBuffLen,
                         (GS16 *)pstGsAxisValueArr, usGsDataNum,
                         g_pstGh30xFrameInfo[GH30X_FUNC_OFFSET_PROTOCOL]);
#endif
    ...
    for (GU8 uchFunCnt = 0; uchFunCnt < GH30X_FUNC_OFFSET_MAX; uchFunCnt++)
    {
        if (g_pstGh30xFrameInfo[uchFunCnt])
        {
            ...
            {
                uchExeFlag = 1;
                if ((GH30X_FUNC_OFFSET_ADT == uchFunCnt) &&
                    (0 == g_uchGh30xIsAdtMode))
                {
                    uchExeFlag = 0;
                }
            }
...
void gh30x_fifo_evt_handler(void)
{
    ...
    while (uchNeedForceReadAgain)
    {
        ...
        gsensor_drv_get_fifo_data(g_UNGh30xRawdataBuf.stRawdataBuf.stGsensorBuf,
                                  &usGsensorNum,
                                  GSENSOR_SOFT_FIFO_BUFFER_MAX_LEN);
        Gh30xNormalizeGsensorSensitivity(
...
// #if (__ALGO_CALC_WITH_DBG_DATA__)
        ...
        usFifoSamplePointNum = Gh30xGetFifoRawdata(
        ...
        // auto led
        uchAutoLedFail = HBD_AutoLedbyFifoInt(
            g_UNGh30xRawdataBuf.stRawdataBuf.unPpgRawdataBuf,
            usFifoSamplePointNum / g_uchNeedReadRawdataCount);
#endif
        ...
        Gh30xDemoFunctionProcess(
            (GU8 *)(g_UNGh30xRawdataBuf.stRawdataBuf.unPpgRawdataBuf),
            usFifoSamplePointNum, g_UNGh30xRawdataBuf.stRawdataBuf.stGsensorBuf,
            usGsensorNum);
    }
    ...
    if (GH30X_FUNCTION_ADT & g_unGh30xDemoFuncMode ||
        GH30X_FUNCTION_SOFT_ADT &
            g_unGh30xDemoFuncMode) // if  ADT fucntion is opened
    {
...
void gh30x_new_data_evt_handler(void)
{
...
#if (__USE_GOODIX_APP__)
    EXAMPLE_DEBUG_LOG_L1(
        "[gh30x_new_data_evt_handler] gh30x_run_mode:%d, app_mode:%d\r\n",
        gh30x_run_mode, goodix_app_start_app_mode);
    if ((goodix_app_start_app_mode) &&
        ((gh30x_run_mode == RUN_MODE_ADT_HB_DET) ||
         (gh30x_run_mode == RUN_MODE_HRV_DET) ||
         (gh30x_run_mode == RUN_MODE_SPO2_DET) ||
         (gh30x_run_mode == RUN_MODE_GETRAWDATA_DET)))
    {
        if (GH30X_AUTOLED_ERR_VAL ==
            HBD_SendRawdataPackageByNewdataInt(&gsensor_data,
                                               __GS_SENSITIVITY_CONFIG__))
        {
...
#endif
    {
        ...
        if (goodix_system_test_mode)
        {
            ...
            EXAMPLE_DEBUG_LOG_L1(
                "got gh30x new data evt, put data to system test module.\r\n");
            GU8 os_test_ret =
                gh30x_systemtest_os_calc(goodix_system_test_os_led_num);
            if (os_test_ret != 0xFF) // test has done
            {
                ...
                if (goodix_system_test_os_led_num < 2 && (!os_test_ret))
                {
                    ...
                    gh30x_systemtest_os_start(goodix_system_test_os_led_num);
                    EXAMPLE_DEBUG_LOG_L1(
                        "system test change to test next led:%d!\r\n",
                        goodix_system_test_os_led_num);
                }
                else
                {
                    //                    goodix_system_test_mode = false;
                    //                    EXAMPLE_DEBUG_LOG_L1("system test has
                    //                    done!\r\n");
                    HBD_Stop();
        ...
        {
            EXAMPLE_DEBUG_LOG_L1(
                "got gh30x new data evt, shouldn't reach here!!\r\n");
            gh30x_handle_calc_unwear_status();
        }
    }
}
...
void gh30x_int_msg_handler(void)
{
    ...
    else if (gh30x_irq_status ==
             INT_STATUS_CHIP_RESET) // if gh30x reset, need reinit
    {
        gh30x_reset_evt_handler();
    }
    else if (gh30x_irq_status ==
             INT_STATUS_FIFO_FULL) // if gh30x fifo full, need restart
    {
...
void gh30x_fifo_int_timeout_msg_handler(void)
{
    ...
    if ((gh30x_irq_status_1 == INT_STATUS_FIFO_WATERMARK) &&
        (gh30x_irq_status_2 == INT_STATUS_INVALID))
    {
        gh30x_fifo_evt_handler();
    }
    else
...
void gh30x_communicate_parse_handler(GS8 communicate_type, GU8 *buffer,
                                     GU8 length)
{
    EM_COMM_CMD_TYPE comm_cmd_type = HBD_CommParseHandler(
        communicate_type, buffer, length); // parse recv data
    if (communicate_type == COMM_TYPE_INVALID_VAL)
    {
        EXAMPLE_DEBUG_LOG_L1("comm_type error, pelase check inited or not, "
                             "@ref gh30x_module_init!!!\r\n");
    }
    else
    {
        EXAMPLE_DEBUG_LOG_L1("parse: cmd[%x-%s], comm_type[%d], length[%d]\r\n",
                             buffer[0],
                             dbg_comm_cmd_string[(uint8_t)comm_cmd_type],
                             communicate_type, length);
    }
    if (comm_cmd_type < COMM_CMD_INVALID)
    {
        ...
        if ((comm_cmd_type == COMM_CMD_ALGO_IN_APP_HB_START) ||
            ...
            (comm_cmd_type ==
             COMM_CMD_ALGO_IN_APP_SPO2_START)) // handle all app mode cmd
        {
            ...
            if (RUN_MODE_ADT_HB_DET == app_start_mode ||
                RUN_MODE_HRV_DET == app_start_mode ||
                RUN_MODE_SPO2_DET == app_start_mode)
            {
                gh30x_module_stop();
            ...
            HBD_FifoConfig(1, HBD_FUNCTIONAL_STATE_ENABLE);
                 ...
                 (comm_cmd_type ==
                  COMM_CMD_ALGO_IN_MCU_SPO2_START)) // handle mcu mode cmd
        {
            ...
            if (RUN_MODE_ADT_HB_DET == mcu_start_mode ||
                RUN_MODE_HRV_DET == mcu_start_mode ||
                RUN_MODE_SPO2_DET == mcu_start_mode)
            {
                gh30x_module_stop();
...
void gsensor_enter_normal_and_clear_buffer(void)
{
    ...
    // gsensor_drv_clear_buffer(gsensor_soft_fifo_buffer,
    // &gsensor_soft_fifo_buffer_index, GSENSOR_SOFT_FIFO_BUFFER_MAX_LEN);
}
...
{
    // gsensor_drv_get_fifo_data(gsensor_soft_fifo_buffer,
    // &gsensor_soft_fifo_buffer_index, GSENSOR_SOFT_FIFO_BUFFER_MAX_LEN);
}
...
void gh30x_systemtest_start(EMGh30xTestItem mode)
{
    ...
    if (mode & 0xc)
    {
        ...
        EXAMPLE_DEBUG_LOG_L1(
            "begin goodix_system_test_mode is %d,mode is %d!\r\n",
...
void gh30x_get_sin_gsensor_data(float Hz, uint8_t range, uint8_t fifomode)
{
    ...
    for (; i < datenum; ++i)
    {
        ...
        gsensor_soft_fifo_buffer[i].sYAxisVal =
            gsensor_soft_fifo_buffer[i].sXAxisVal;
        gsensor_soft_fifo_buffer[i].sZAxisVal =
            gsensor_soft_fifo_buffer[i].sXAxisVal;
        ++gsensor_soft_fifo_buffer_index;
        ++ntime;
    }
...
const double ra_arry[8] = {1,
                           ...
                           -3.67327825092135,
                           0.558034825523797};
const double rb_arry[8] = {0.747017285960491, -4.48210371576294,
                           11.2052592894074,  -14.9403457192098,
                           11.2052592894074,  -4.48210371576294,
                           0.747017285960491}; // impulse response
...
unsigned char HBDTEST_OTP_Check(ROMAHBD_Interfcae *ROMAHBD_Str,
...
#if __SYSTEM_TEST_SUPPORT__
static void CalcNoise(int *pRawdata, short int nCount,
                      HBDTEST_ROMAHBData *pHBDataBuff)
{
    ...
    //    for (rawdataVec::const_iterator it = rawdataVector.begin(); it !=
    //    rawdataVector.end(); ++it)
    //    {
    //        nSum += *it;
    //    }
...
double design(const double val)
{
    if (g_UNGh30xRawdataBuf.stFactoryRawdata.HighPassFilters.m_bFirstDataFlag)
    {
        ...
        for (char i = 0; i < 7; i++)
        {
            g_UNGh30xRawdataBuf.stFactoryRawdata.HighPassFilters.m_vecA[i] =
                ra_arry[i];
            g_UNGh30xRawdataBuf.stFactoryRawdata.HighPassFilters.m_vecB[i] =
                rb_arry[i];
        }
        for (char i = 0; i < 6; i++)
        {
            g_UNGh30xRawdataBuf.stFactoryRawdata.HighPassFilters
                .m_vecDataIn[i] = val;
            g_UNGh30xRawdataBuf.stFactoryRawdata.HighPassFilters
                .m_vecDataOut[i] = 0.0;
        }
        ...
        //        std::transform(m_vecDataIn.begin(), m_vecDataIn.end(),
        //        m_vecDataIn.begin(), [skip](T v)->T{
        //            return (v + skip);
        //        });
        g_UNGh30xRawdataBuf.stFactoryRawdata.HighPassFilters.m_bFirstDataFlag =
            0;
    }
    ...
    for (int i = 0; i < 6; ++i)
    {
        sum += (g_UNGh30xRawdataBuf.stFactoryRawdata.HighPassFilters
                    .m_vecB[i + 1] *
                g_UNGh30xRawdataBuf.stFactoryRawdata.HighPassFilters
                    .m_vecDataIn[6 - i - 1]);
    }
    double dataBuffer1 =
        g_UNGh30xRawdataBuf.stFactoryRawdata.HighPassFilters.m_vecB[0] * val +
        sum;
    ...
    double dataOut =
        ...
        g_UNGh30xRawdataBuf.stFactoryRawdata.HighPassFilters.m_vecA[0];
    ...
    for (int j = 0; j < 6; j++)
    {
        ...
        {
            g_UNGh30xRawdataBuf.stFactoryRawdata.HighPassFilters
                .m_vecDataIn[j - 1] = g_UNGh30xRawdataBuf.stFactoryRawdata
                                          .HighPassFilters.m_vecDataIn[j];
            g_UNGh30xRawdataBuf.stFactoryRawdata.HighPassFilters
                .m_vecDataOut[j - 1] = g_UNGh30xRawdataBuf.stFactoryRawdata
                                           .HighPassFilters.m_vecDataOut[j];
        }
    }
    g_UNGh30xRawdataBuf.stFactoryRawdata.HighPassFilters.m_vecDataIn[5] = val;
    g_UNGh30xRawdataBuf.stFactoryRawdata.HighPassFilters.m_vecDataOut[5] =
        dataOut;
...
unsigned char
HBDTEST_Check_Rawdata_Noise(int *_inDatabuff, const unsigned char lenth,
                            const HBDTEST_ROMAHBData *_inRawdataStr)
{
    ...
    {
        g_UNGh30xRawdataBuf.stFactoryRawdata.unTempBufForStatistic[i] =
            (int)design(_inDatabuff[i]);
    }
    CalcNoise(
        (GS32 *)g_UNGh30xRawdataBuf.stFactoryRawdata.unTempBufForStatistic,
        lenth, &ROMAHBStr);
    int nLpNoise_L1 = ROMAHBStr.nLpNoise;
    CalcNoise(_inDatabuff, lenth, &ROMAHBStr);
    ROMAHBStr.nLpNoise = nLpNoise_L1;

    if ((_inRawdataStr->nNoiseRawDataMax < ROMAHBStr.nNoiseRawDataAvr) ||
        (_inRawdataStr->nNoiseRawDataMin > ROMAHBStr.nNoiseRawDataAvr))
    {
        return EN_RAWDATA_OUT_RANGE; // err
    }
    if ((_inRawdataStr->nLpNoise < ROMAHBStr.nLpNoise) ||
        (0 == ROMAHBStr.nLpNoise))
    {
...
unsigned char HBDTEST_Check_CTRandNoise(int *_inDatabuff,
                                        const unsigned char lenth,
                                        HBDTEST_ROMALEDCheckData *data)
{
    if ((lenth == 0) || (lenth > 100))
        return EN_PARAM_FAIL; // err
    HBDTEST_ROMAHBData ROMAHBStr;
    ...
    //    if(
    //    (_inRawdataStr->nNoiseRawDataMax<ROMAHBStr.nNoiseRawDataAvr)||(_inRawdataStr->nNoiseRawDataMin>ROMAHBStr.nNoiseRawDataAvr))
    //    {
    //        return 1;//err
    //    }
    data->_res._CTR = ROMAHBStr.nNoiseRawDataAvr * 800000.0f / 65536.0f /
                      data->_param._ledCurr / data->_param._ledResis;
    if (data->_res._CTR < data->_std._ctrStd)
    {
        return EN_CTR_UNDER_STD;
    }
    if ((data->_std._noiseStd < ROMAHBStr.nLpNoise) ||
        (0 == ROMAHBStr.nLpNoise))
    {
        return EN_NOISE_UNDER_STD; // err
    }
...
unsigned char HBDTEST_Check_LeakandRatio(int *buff, const unsigned char lenth,
                                         HBDTEST_ROMALEDCheckData *data)
{
    int sum = 0;
    ...
    data->_res._leak = ((float)sum) / lenth * 800000.0f / 65536.0f /
                       data->_param._ledCurr / data->_param._ledResis;
