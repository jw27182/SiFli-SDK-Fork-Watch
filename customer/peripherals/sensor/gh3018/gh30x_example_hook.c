...
void Gh30xInitHookFunc(void)
{
    ...
    {
        GH3X2X_SEND_MSG_ALGO_CFG_INIT(g_pSTGh30xFrameInfo,
...
/**
 * @fn      void gh3x2x_algorithm_get_io_data_hook_func(const STGh30xFrameInfo *
 * const pstFrameInfo)
 *
...
void Gh30xAlgorithmGetIoDataHookFunc(const STGh30xFrameInfo *const pstFrameInfo)
{
    ...
    EXAMPLE_DEBUG_LOG_L2(
        "[IO_DATA]FunId= 0x%X,cnt = %d,flg = 0x%X, x = %d, y = %d, z= %d, ",
        pstFrameInfo->unFunctionID, pstFrameInfo->punFrameCnt[0],
        pstFrameInfo->pstFunctionInfo->uchFrameFlag,
        pstFrameInfo->pusGsensordata[0], pstFrameInfo->pusGsensordata[1],
        pstFrameInfo->pusGsensordata[2]);

    for (GU8 uchChnlCnt = 0;
         uchChnlCnt < pstFrameInfo->pstFunctionInfo->uchChnlNum; uchChnlCnt++)
    {
    ...
    #endif
        EXAMPLE_DEBUG_LOG_L2(
            "chnl%d: id= 0x%d, adj= %d, gn= %d, bg= %d, cur= %d, raw= %d",
            uchChnlCnt, pstFrameInfo->unFunctionID, adj_flag, gain, bg,
            pstFrameInfo->pchDrvCurrentCode[uchChnlCnt], RawdataCode);
    }
    EXAMPLE_DEBUG_LOG_L2("\r\n");
#endif
}
...
static const GF32 highPassFilterCoefA[] = {8192, -14310,  6511.2,
                                           8192, -6490.8, 0};
static const GF32 highPassFilterCoefB[] = {7254.2, -14505,  7254.2,
                                           7341.4, -7341.4, 0};
static const GF32 LowPassFilterCoefA[] = {8192, -10315,  4505.3,
                                          8192, -4191.5, 0};
static const GF32 LowPassFilterCoefB[] = {642.65, 1096.7, 642.65,
                                          2000.3, 2000.3, 0};
...
static GF32 HPXBufferX[ADT_CONFIRM_FILTER_BUF_SIZE],
    HPYBufferX[ADT_CONFIRM_FILTER_BUF_SIZE];
static GF32 HPXBufferY[ADT_CONFIRM_FILTER_BUF_SIZE],
    HPYBufferY[ADT_CONFIRM_FILTER_BUF_SIZE];
static GF32 HPXBufferZ[ADT_CONFIRM_FILTER_BUF_SIZE],
    HPYBufferZ[ADT_CONFIRM_FILTER_BUF_SIZE];
static GF32 LPXBuffer[ADT_CONFIRM_FILTER_BUF_SIZE],
    LPYBuffer[ADT_CONFIRM_FILTER_BUF_SIZE];
...
void gh3011_algo_init_hook(EMFunctionID function_id, GU8 buffer_len,
                           GS32 *algo_param_buffer)
{
    ...
    goodix_hrv_config stHrvConfig = {1, 200, {200000, 100000, 30000, 30000}};
#endif

#if (__HBD_SPO2_ALGORITHM_ENABLE__)
    goodix_spo2_config stSpo2Config = {
        1,  100, -21742, -201492, 1077714,    // 0xffffab12,0xfffcecec,0x1071d2
        1,  0,   30,     15,      4,       3, // 6. Acc相关
        0,  0,                                // ctr相关
    #if __HBD_ALGORITHM_VERTION__ == __HBD_ALGORITHM_VERTION_BASIC__
        30, 20,  10,     4,       6,       4, 5, // 出值策略
    ...
    GS32 liNadtCfg[33] = {0,  0,   5, 4,  60, 25, 0,   1, 1,  40,  140,
                          10, 180, 1, 10, 1,  10, 100, 1, 10, 1,   4,
                          3,  3,   1, 30, 90, 3,  10,  7, 0,  400, 5};
#endif
    ...
    case GH3011_FUNC_OFFSET_HR:
        if (buffer_len == 3)
        {
            ...
            stHbaConfig.scence = (hba_scenes_e)
                algo_param_buffer[1]; // user can config scense here
            stHbaConfig.mode = (hba_test_mode)
                algo_param_buffer[2]; // user can config test mode here
            ...
            stHbaConfig.senseless_mode_duration =
                gsthbdCfg.senseless_mode_duration;
            stHbaConfig.hba_latest_output_time =
                gsthbdCfg.hba_latest_output_time;
            stHbaConfig.hba_earliest_output_time =
                gsthbdCfg.hba_earliest_output_time;
    #endif
            EXAMPLE_DEBUG_LOG_L1_HOOK(
                "GH3011_FUNC_HR stHbaConfig = %d %d %d %d\n\r", stHbaConfig.fs,
                stHbaConfig.scence, stHbaConfig.hba_latest_output_time,
                stHbaConfig.valid_channel_num);
    #if (__HBD_DOUBLE_CORE_ENABLE__)
            // ipc_trans(trans_data, trans_len , recv_data, rec_len);
    ...
    case GH3011_FUNC_OFFSET_HRV:
        if (buffer_len == 1)
        {
            ...
            EXAMPLE_DEBUG_LOG_L1_HOOK(
                "GH3011_FUNC_OFFSET_HRV stHrvConfig.fs = %d %d %d %d %d %d\n\r",
                stHrvConfig.fs, stHrvConfig.need_ipl, stHrvConfig.acc_thr[0],
                stHrvConfig.acc_thr[1], stHrvConfig.acc_thr[2],
    ...
    case GH3011_FUNC_OFFSET_NADT:
        if (buffer_len == 4)
        {
            ...
            EXAMPLE_DEBUG_LOG_L1_HOOK(
                "nadt para %d, %d, %d, %d\r\n", algo_param_buffer[0],
                algo_param_buffer[1], algo_param_buffer[2],
                algo_param_buffer[3]);
            ...
            NADT_Control(NADT_CONFIG_ADT_ONLY_SLEEP_TYPE,
                         &guiNadtSleepOnlyAdtStatus);
        #endif
            NADT_ProcInit();
    #endif
        }
                                ...
                                ST_ALGO_RESULT *algo_result)
{
    ...
    case GH3011_FUNC_OFFSET_HRV:
        ...
        if (ret == GX_HRV_ALGO_UPDATE)
        {
            ...
            EXAMPLE_DEBUG_LOG_L1_HOOK(
                "stHrvResult = %d %d %d %d %d %d\n\r", stHrvResult.rri[0],
                stHrvResult.rri[1], stHrvResult.rri[2], stHrvResult.rri[3],
                stHrvResult.rri_confidence, stHrvResult.rri_valid_num);
        }
        break;
#endif
#if (__HBD_SPO2_ALGORITHM_ENABLE__)
    case GH3011_FUNC_OFFSET_SPO2:
        ...
        if (stSpo2Result.final_calc_flg == 1 && ret == GX_ALGO_SPO2_SUCCESS)
        {
            ...
            algo_result->snResult[0] =
                (GS32)(((float)stSpo2Result.final_spo2 / 10000 + 0.5f));
    ...
    // ipc_trans(trans_data, trans_len , recv_data, rec_len);
        ...
        EXAMPLE_DEBUG_LOG_L1_HOOK("NADT_Proc lNadtPacketInfoArr = %d , %d, %d, "
                                  "%d, %d, %d\n\r lNadtResult = %d, %d\n\r",
                                  lNadtPacketInfoArr[0], lNadtPacketInfoArr[1],
                                  lNadtPacketInfoArr[2], lNadtPacketInfoArr[3],
...
* Input:    uchScenario: 0~24,see hba_scenes_e
                         others: fixed 0(default) and return
HBD_RET_PARAMETER_ERROR
...
******************************************************************/
void HBD_HbaTestModeConfig(hba_test_mode emHbaTestMode,
                           GU16 usSenselessModeStep,
                           GU16 usSenselessModeDuration)
{
...
* Return: HBD_RET_OK=success,
******************************************************************/
GS8 HBD_HbAlgoOutputTimeConfig(GS32 nHbaLatestOutputTime,
                               GS32 nHbaEarliestOutputTime)
{
...
/////////////////////////////////////
/// init
void IIRFilter2Init(ST_IIR_PARAM *pstIIRInfo, const GF32 *pfCoefA,
                    const GF32 *pfCoefB, GF32 *pfXbuff, GF32 *pfYbuff,
                    GS32 lLen, GF32 fThr)
{
...
static GF32 HPFilter(const GF32 b[], const GF32 a[], GF32 x[], GF32 y[],
                     GF32 dDataIn)
{
    GF32 dSum2 = 0;
...
GF32 IIRFilterEx(ST_IIR_PARAM *pstIIRInfo, GF32 fDataIn)
{
    ...
    if ((fDiff == 0 && (fDataIn > 16000 || fDataIn == 0)) ||
        NADT_MATH_ABSF(fDiff) >= pstIIRInfo->fThr)
    {
        pfXbuff[0] += fDiff;
        pfXbuff[1] += fDiff;
    }
    ...
    for (int i = 0; i < 2; i++)
    {
        fDataOut = HPFilter(pfCoefB + i * 3, pfCoefA + i * 3, pfXbuff + i * 2,
                            pfYBuff + i * 2, dDataIn1);
...
void ADTConfirmAlgoInit(GU32 ulNewAdtPpgThrVal)
{
    ...
    IIRFilter2Init(&highPassFilterX, highPassFilterCoefA, highPassFilterCoefB,
                   HPXBufferX, HPYBufferX, 3, 4001);
    IIRFilter2Init(&highPassFilterY, highPassFilterCoefA, highPassFilterCoefB,
                   HPXBufferY, HPYBufferY, 3, 4001);
    IIRFilter2Init(&highPassFilterZ, highPassFilterCoefA, highPassFilterCoefB,
                   HPXBufferZ, HPYBufferZ, 3, 4001);
    IIRFilter2Init(&lowPassFilter, LowPassFilterCoefA, LowPassFilterCoefB,
                   LPXBuffer, LPYBuffer, 3, 4001);
...
GU8 ADTConfirmAlgoCalc(GS32 *plRawdataBuff)
{
    ...
    GU8 uchWearThrCnt = 0;
    GU32 ulAdtNowFlag = 0;
    GU32 ulPpgRawdataVal =
        (GU32)((plRawdataBuff[0] >> 7) & 0x0001FFFF); // just use 17bits
    ...
    if (g_uchAdtGsCalcThrCntMaxIndex < g_uchAdtGsCalcThrCntMax)
    {
        ...
        {
            fAccx = IIRFilterEx(&highPassFilterX,
                                plRawdataBuff[2]); // gx data input hp filter
            fAccy = IIRFilterEx(&highPassFilterY,
                                plRawdataBuff[3]); // gy data input hp filter
            fAccz = IIRFilterEx(&highPassFilterZ,
                                plRawdataBuff[4]); // gz data input hp filter
            fAccAmpVal = sqrt(fAccx * fAccx + fAccy * fAccy + fAccz * fAccz);
            fAccAmpLpfVal =
                IIRFilterEx(&lowPassFilter, fAccAmpVal); // gs input lp filter
        }
        ...
        // ulPpgRawdataVal = %d g_ulAdtPpgThrVal = %d\n\r",fAccAmpLpfVal,
        // g_usAdtGsThrVal, ulPpgRawdataVal, g_ulAdtPpgThrVal);
        if (g_uchAdtGsCalcThrCntMaxIndex >= g_uchAdtGsCalcThrCntMax)
        {
            ...
            if ((uchWearThrCnt >= g_uchAdtGsCalcGreaterThanThrCnt) &&
                (g_uchPpgThrCnt < 5))
            {
                uchRet = 0x11;
            }
...
* Return: None
******************************************************************/
void HBD_AdtConfirmConfig(GU16 usAdtConfirmGsThrVal,
                          GU8 uchAdtConfirmGsCalcThrCntMax,
                          GU8 uchAdtConfirmGsCalcThrCnt)
{
