...
void rt_tick_increase_by_step(rt_tick_t step);
rt_tick_t  rt_tick_from_millisecond(rt_int32_t ms);
rt_tick_t rt_tick_get_millisecond(void);
/** get system time, default is to return rt_tick_get(), could be overrided by user defined time function */
...
struct rt_timer *rt_timer_next_timer(void);
#ifdef RT_USING_TIMER_SOFT
struct rt_timer *rt_timer_next_soft_timer(void);
...
void rt_system_timer_retention(void);

rt_err_t rt_system_retention_register(
    rt_timer_t timer,
    void (*notify)(rt_timer_t timer, void *parameter));
#endif
#endif
...
typedef rt_ubase_t (*rt_compressor_cb_t)(void *dst, const void *src, rt_ubase_t src_size, rt_ubase_t dst_size);
rt_err_t rt_thread_stack_backup(struct rt_thread *thread, rt_uint8_t *buf,
                                rt_uint32_t max_size, rt_uint32_t *used_size);
rt_err_t rt_thread_stack_restore(void *instance, rt_uint8_t *buf, rt_uint32_t size, rt_compressor_cb_t decompressor_cb);
...
rt_size_t rt_mem_size(void *ptr);
...
void *rt_memheap_calloc(struct rt_memheap *heap, rt_size_t count, rt_size_t size);
...
void rt_hexdump(const char *tag, rt_size_t width, rt_uint8_t *buf, rt_size_t size);

void rt_show_version(void);
void rt_set_version(const char *ver);
const char *rt_get_version(void);
...
void rt_assert_set_hook_rom(void (*hook)(const char *ex, const char *func, rt_size_t line));

/* Not mark as NO_RETURN as it breaks backtrace */
